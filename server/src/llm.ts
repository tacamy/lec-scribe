 ...
 *   2. outline … 整えた本文全体から、講義全体の要点と「話題の区切り」（見出し・要点・開始位置）を作る
...
export function buildPrompt(sections: readonly PolishInput[]): string {
  const body = sections
    ...
    .join('\n\n');
  return `あなたは大学講義のノートを作る編集者です。以下は講義音声の自動文字起こし（話し言葉）を、画面が切り替わったところで区切ったものです。各セクションについて text を作ってください。
...
/** 整えた本文全体から、講義全体の要点と話題の区切りを頼むプロンプト */
export function buildOutlinePrompt(sections: ReadonlyArray<{ id: string; text: string }>): string {
  const body = sections
    .map((s) => `<<<PART id="${s.id}">>>\n${s.text.trim() || '（発話なし）'}\n<<<END>>>`)
    .join('\n\n');
  return `あなたは大学講義のノートを作る編集者です。以下は講義の文字起こしを読みやすく整えた本文を、画面が切り替わったところ（スライドや操作画面の変化）で区切って順に並べたものです。画面の切り替わりは話の区切りと一致しないことが多い（操作のデモで画面が細かく変わる、同じスライドで別の話題に移る、など）ので、話題の区切りは内容から判断してください。次の 2 つを作ってください。

1. overview: 講義全体の要点。日本語の箇条書きで 5〜12 項目、各項目は 1 文。記号や番号は付けない。
2. topics: 話題のまとまり。本文の順に並べ、各要素は次の 3 つ。
   ...
   最初の topic の startId は最初の部分の id にしてください。話題は細かく割りすぎず、10 分の講義なら 2〜4 個、90 分なら 6〜15 個が目安です。「（発話なし）」の部分は前の話題に含めてください。
...
/** 整えた本文全体から、講義全体の要点と話題の区切りを 1 回の呼び出しで作る。失敗しても notes.md は作れる */
