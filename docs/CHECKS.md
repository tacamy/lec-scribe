...
目的: スライドごとに画像とその間の発話が並んだ講義ノートができること（SPEC §13.4, D-14）。
...
| 2026-09-08 | 7 | MacBook Pro M3 Max | OK | 実講義 131 秒の録音で送信 → 文字起こし → `~/LecScribe/…/transcript.srt` まで通った（35 区間、動画時刻に変換済み）。初回は `large-v3` のダウンロード（1.25 GB、約 2 分）と CoreML の初回コンパイルで文字起こし段階が 441 秒。キャッシュ済みの 2 回目は 41.9 秒（モデル読み込み込みで約 3 倍速）。16 分の講義では Done まで約 5 分（約 3.2 倍速、90 分なら 25〜30 分の見込み）。同じ音声（966 秒）を `large-v3_turbo` で処理すると初回 846 秒（ダウンロード・コンパイル込み）、2 回目 248 秒（約 3.9 倍速）。区間数 297 で同じ、冒頭の本文も同一で精度差は見えず。速度差が 2 割程度なので既定は `large-v3` のまま（`--model large-v3_turbo` で切替可）。エンコーダ・デコーダを GPU（`--*-compute-units cpuAndGPU`）にすると 2 回目 315 秒で Neural Engine より遅く、区間の切り方も変わった（177 区間）ため不採用 |
