...
| 2026-09-08 | 7 | MacBook Pro M3 Max | OK | 実講義 131 秒の録音で送信 → 文字起こし → `~/LecScribe/…/transcript.srt` まで通った（35 区間、動画時刻に変換済み）。初回は `large-v3` のダウンロード（1.25 GB、約 2 分）と CoreML の初回コンパイルで文字起こし段階が 441 秒。キャッシュ済みの 2 回目は 41.9 秒（モデル読み込み込みで約 3 倍速。90 分の講義なら 15〜30 分の見込み。`large-v3_turbo` は未比較） |
| | 8 | | 未実施 | |
