...
- v0.8（2026-09-17）: パネルの Video 行の再生位置を、検知スクリプトの報告の間も進める（§15.1。数字を等幅にし、動画の長さで止め、開始処理中は「—」）。ポップアップの Start 前の動画の表示を、状態が変わっても消さない（§15.1）。同じ場面のまとめで、まとまりの前の方の画像とも比べ、読み取りが安定しない場面では文字を拒否の根拠にしない（§13.4b）。LLM の呼び出しを 10 分で打ち切り、スリープ復帰後に自動でやり直す（§13.5）。
...
- 返答が壊れた、または一部の節が返ってこなかったときは、そのバッチを半分に分けて 1 度だけやり直す。分けても直らなければその分だけ諦め、`notes.md` には文字起こしのまま載せる。1 回の呼び出しは 10 分で打ち切る（2026-09-17。普段は 1〜4 分で返る。Mac がスリープすると途中の要求が途切れ、codex は諦めずに待ち続けるので、打ち切らないとそのセッションは永久に「ノート作成中」のままで、処理中の件数も戻らず `agent restart` も断られる。10 章で 2 時間止まった。スリープ中はタイマーも止まるが、復帰した瞬間に期限切れとして処理されるので、復帰と同時に打ち切って分けてやり直す。失われるのは寝ていた時間だけで、手は要らない。利用者の「中止」とは理由を分けて記録する）。
