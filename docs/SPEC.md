...
Video 行の再生位置は、検知スクリプトの報告（5 秒ごとの定期報告と、再生・停止・シークなどのイベント時。§8）をそのまま出すと 5 秒に 1 回しか進まないので、再生中はパネル側で報告からの経過に再生速度を掛けて足し、「経過」と同じ 0.5 秒の拍子で描き直す（2026-09-17、`videoTimeNow`）。検知スクリプトにも service worker にも手を入れず、通信も保存も増やさない。停止・バッファリング・シークはイベントで即報告されるのでずれは次の報告で直り、報告が途切れたときのために足す幅は 6 秒で止める。同じ行の「変化 x%」は検知の判定結果なので補えず、報告のたびに変わる。
