...
- サンプル数の半分以上で変わった画素は「動き続けている」とみなし、**切り替えの判定（`diffPrev`）からも最終状態の比較（`diffFromSaved`）からも外す**。差分率の分母も静止部分の画素数にする。
...
これで、講師が動いただけのキャプチャ（上記 58 組のうち 12 組）は撮られなくなり、閾値を上げずに済む。
