...
- 文字起こしはできたがノートを整えられなかった行には、注意書きを 1 行出す（§13.5、2026-09-20）。拡張は処理が `done` になったとき、サーバーの `result.notes` が `false`（全体）か、`true` で `notesError` がある（一部）かを `status.json` の `notesProblem`（`failed` / `partial`）と `notesError` に残し、「やり直す」で整えられたら消す。処理中・送信待ちの間と「データなし」の行には出さない
