...
- v0.8（2026-09-17）: パネルの Video 行の再生位置を、検知スクリプトの報告の間も進める（§15.1。数字を等幅にし、動画の長さで止め、開始処理中は「—」）。ポップアップの Start 前の動画の表示を、状態が変わっても消さない（§15.1）。同じ場面のまとめで、まとまりの前の方の画像とも比べ、読み取りが安定しない場面では文字を拒否の根拠にしない（§13.4b）。LLM の呼び出しにサーバー側の打ち切りを付けない理由を記録（§13.5、2026-09-18）。セキュリティレビューを受けて、承認済みの拡張でもダイアログなしではトークンを返さず、ダイアログの既定を「許可しない」に（D-10、§12.2）、`slides.json` の画像名をサーバーで確かめ、新しく作るフォルダを本人だけが読める権限にし、トークンをログに出さない（§18、2026-09-18）。
...
| POST | `/pair` | `{ name? }`。認証不要だが `Origin: chrome-extension://<id>` 必須。macOS のダイアログで承認されると `{ paired: true, token }`（承認済みの拡張でも毎回ダイアログを出し、「許可」で新しいトークンに替える。2026-09-18）。拒否は 403、ダイアログ表示中は 429 |
