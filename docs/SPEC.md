...
見出しの時刻は動画時刻。スライドに属するセグメントを結合し、句点で改行する。最初のスライドより前の発話は「冒頭（スライドなし）」の節に入れる。自動検知で保存したスライド（`reason: change`）は切り替えの 1〜2 秒後に確定するため、表示開始を `videoTime - 1.5` 秒とみなして割り当てる（初回・手動はそのまま）。`transcript.json` の各区間にも `slide` を入れる。
