...
- サンプルが 4 つに満たない間（録音開始直後）と、静止部分が全体の 10% を切ったとき（画面全体が映像）は、従来どおり全画素で比べる。
...
あわせて、映像中心の画面では最短キャプチャ間隔を `detect.footageMinIntervalMs`（既定 15 秒）まで延ばす。同じ場面が続く間に何枚も撮らないため。切り替わりが落ちるわけではなく、間隔が空いたあとの安定した時点で保存される。スライド中心の画面は従来どおり `minShotIntervalMs`（2 秒）。

閾値を実データで詰められるよう、保存したスライドには判定に使った数値を `slides.json` の `trigger`（`diffPrev` / `diffSaved` / `cells` / `stillFraction`）として残す。

### 9.1d 見た目が同じ場面は撮らない（2026-09-09）

映像が中心の動画（スライドではなく被写体を映している場面）では、被写体が動くだけで画素の差は大きく出る。3 章の録音では、隣り合うキャプチャの差が 160×90 で 12〜45% あるのに、32×18 に均すと 2〜9% しかない組が 4 つあった（ほかの 37 組は 23% 以上）。人の目には同じ場面に見える。

そこで保存を決める直前に、候補と最後に保存した画像を 32×18 に均して比べ、差が `detect.lookAlikeThreshold`（既定 12%）未満なら保存しない。細かい動きは均されて消え、場面の違いだけが残る。

ただしこの判定は **静止部分が画面の半分未満のとき（映像中心の画面）だけ** 使う。スライド中心の画面では、レイアウトが同じで文字だけ違うスライドが「見た目が同じ」と判定されてしまうため。
...
- 本文が決まり文句（「ご視聴ありがとうございました」「チャンネル登録お願いします」など。同じ文の繰り返しも 1 つとみなす）だけで、しゃべる速さから考えて長すぎるもの。日本語の話し言葉は 1 文字 0.15〜0.2 秒なので、0.35 秒/文字（かつ 3 秒以上）を超えたら実際には言っていないとみなす。「ご視聴ありがとうございました」（14 文字）なら 4.9 秒以上が対象。普通の速さで言っている分は残す（2026-09-09、3 章の録音で 7.4 秒の同じ文句が残っていたため追加）
