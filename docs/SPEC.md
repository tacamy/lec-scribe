...
- サーバーは **約束の版** `API_VERSION`（`server/src/app.ts`、整数）を `/health` の `api` で返す。上げるのは HTTP の約束が変わったときだけ（拡張が送る・受け取る項目が増える、意味が変わる）。内部の改善やノートの作り方の変更では上げない。`1` = 2026-09-11（cancel の `force`、status の 404、title 先頭のフォルダ名まで）。`2` = 2026-09-11（`/health` に `vision`。#17。拡張が必要とする最低の版は 1 のまま。見せるだけの項目なので、無いサーバーには古い警告を出さず、その行を出さないだけ）
...
| メソッド | パス | 内容 |
|---|---|---|
| GET | `/health` | `{ ok, version, api, commit, ffmpeg, whisperkit, vision, model, llm, authorized, paired, processing }`。`vision` は見た目の判定の補助コマンドが使えるか（api 2 から。ディスクを見るだけでビルドはしない。Vision を使わない設定なら null。#17）。`api` は拡張との約束の版（§12.1c）、`commit` は動いているコードの短い SHA（診断用。分からなければ null）。ffmpeg / whisperkit の有無は `resolveBin` が覚えている（1 分で忘れる。毎回 PATH を歩いていた #9） |
...
- 次に、**見た目の距離**（macOS の Vision、`VNGenerateImageFeaturePrintRequest`。2026-09-10）で比べる。画素や色の分布では分けられなかった「同じ写真の続き」「メニューを開いただけ」「少しスクロールしただけ」を、人の目に近い基準で拾う。Swift の小さな補助コマンド（`server/tools/imagefp.swift`）を `swiftc` でビルドして `~/.lec-scribe/bin/` に置き（常駐サーバーは起動時に作る。§12.1b。間に合わなければ最初の処理で待つ）、画像パスを渡して距離の行列を受け取る。Mac の中だけで動きトークンを使わない。`swiftc`（Xcode Command Line Tools）がなければこの判定を飛ばす。ビルドは 3 分で打ち切る（CLT が無い Mac にも `/usr/bin/swiftc` があり、「開発者ツールをインストールしますか」のダイアログを出したまま返らないことがある。打ち切らないと、あとから補助コマンドを待つ文字起こしも一緒に止まる）。**作れなかった結果は覚えない**（次に呼ばれたらやり直す。常駐サーバーは何日も動くので、起動時にたまたま駄目だっただけで、その間ずっと諦めることになる）。作れているかは `/health` の `vision` で分かり、拡張の設定画面の「接続を確認」に「見た目の判定（Vision）: あり / なし」と出る（2026-09-11、#17。`install.sh` が前もってビルドしていたときはターミナルに結果が出ていたが、起動時に移して見えなくなったため）。
