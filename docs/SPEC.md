...
- サンプルの 1/4 以上（かつ 3 回以上）変わった画素は「動き続けている」とみなし、**切り替えの判定（`diffPrev`）からも最終状態の比較（`diffFromSaved`）からも外す**。差分率の分母も静止部分の画素数にする。
...
これで、講師が動いただけのキャプチャは撮られなくなり、閾値を上げずに済む。3 章の録音（59 枚）でも同じ測り方で 58 組中 12 組が「静止部分の差 2.5% 未満」＝ 動いただけだった。静止部分の差は 3.23% と 10.29% の間で大きく空くので、切り替えの閾値 2.5% はこの間に収まる。
