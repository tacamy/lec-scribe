...
- v0.8（2026-09-17）: パネルの Video 行の再生位置を、検知スクリプトの報告の間も進める（§15.1。数字を等幅にし、動画の長さで止め、開始処理中は「—」）。ポップアップの Start 前の動画の表示を、状態が変わっても消さない（§15.1）。同じ場面のまとめで、まとまりの前の方の画像とも比べ、読み取りが安定しない場面では文字を拒否の根拠にしない（§13.4b）。LLM の呼び出しにサーバー側の打ち切りを付けない理由を記録（§13.5、2026-09-18）。セキュリティレビューを受けて、接続の承認を見直した（D-10、§12.2。トークンは承認 1 回ごとに 1 つ、押し直しはトークンを添えればダイアログなし、それ以外は毎回ダイアログで既定は「許可しない」、設定画面に「接続を解除」と `POST /unpair`。api 3）。`slides.json` の画像名をサーバーで確かめ、新しく作るフォルダを本人だけが読める権限にし、トークンをログに出さない（§18、2026-09-18）。設定画面からポートとトークンの欄を外し、ポートをほかのアプリが使っているときは相手を止めるよう案内する（§12.1d、§15.2、2026-09-18）。位置づけを明記（§3.0、2026-09-20。動画を見ながらノートを取る汎用の道具、個人の学習用、利用するサイトの規約に従う）。画面を少しスクロール・パンしただけの画像をまとめる（§13.4b、2026-09-20）。前の画面に 10 秒未満だけ戻った画像を載せない（§13.4bノートを整えられなかった講義を一覧で知らせる（§11.3、§13.5、2026-09-20）。やり直しの中止はやり直す前の状態に戻し、初回の処理のノート作成中の中止は録音を消さずに文字起こしのままのノートで完了にする（§11.3、api 4保存済みセッションの一覧は 10 件まで見せ、残りは「すべて表示」で開く（§15.1、2026-09-20）。別ドメインの埋め込みの警告は、見えていて動画プレイヤーが入りうる大きさの iframe があるときだけ出す（§8.1、2026-09-21）。アイコンでサイドパネルを直接開く形に変え、ポップアップをやめた（D-12、2026-09-21）。Downloads への書き出しを `downloads` 権限ごと廃止（§11.2、2026-09-24）。画面を大きくスクロールしただけの組（行ごとの色の並びで見る）と、同じショットの続き（写真・映像で、切り替えの瞬間の変化が小さかった画像）もまとめる（§13.4b、2026-09-22）。ほぼ一色の画像（真っ黒など）は載せない（§13.4b、2026-09-22）。凹凸の少ない白地のページを少し送った組もまとめる（§13.4b、2026-09-24）。撮影した紙面の上で手（指）が動いただけの組もまとめ、その帯では写真同士の規則を使わない（§13.4b、2026-09-24）。
- v0.7（2026-09-11）: サーバーの自動更新（§12.1b）。拡張とサーバーの版の突き合わせ（§12.1c。`/health` の `api` / `commit`）。検知の精度（動き続ける画素のマスク §9.1b、変化の広がり §9.1、カット §9.1c、色の分布 §9.1d）。サーバー側で同じ場面の画像をまとめる（§13.4b。macOS の Vision による画像比較と文字認識）。画像を文の途中に挟まない（§13.4）。ノート作成の結果を残して使い回す（§13.5b）、1 回に送る本文を 12,000 字に（§13.5）。出力フォルダを `<タイトル>_<日時-ID>` に（§14）。一覧の「破棄」を「削除 / 中止 / 非表示」に分け、全件表示・ツールチップ・確認ダイアログを入れた（§11.3、§15.1）。更新の後始末（Vision の補助コマンドのビルド）はサーバーの起動時に集める（§12.1b、#10）。launcher はリポジトリの `start.sh` に渡すだけにし、起動コマンドを git で届くようにした（§12.1b、#10）。`/health` の `vision` で補助コマンドの状態と失敗の理由を設定画面とターミナルに出す（§12.2、#17。api 2）。
...
- **動画を見ながらノートを取るための汎用の道具。** 特定のサイトや配信事業者向けに作らない。対象は「ページに `<video>` で埋め込まれた動画」で、§2 のプレーヤー名は最初に動作を確かめた環境を書いているだけ
- **個人の学習用。** 自分が視聴できる動画を見ながら、自分のためのノートを作る。録音・画像・ノートを配ったり公開したりする使い方は想定しない
- **利用するサイトの規約に従うのは使う人の責任。** 録音や画面の保存を禁じているサイトでは使わないよう、README の「使う前に」、ストアの詳細説明（`docs/STORE.md`）、拡張の未接続の画面（§15.1）に明記する。manifest の `description`（ストアの概要、chrome://extensions のカード）は 3 行で切れるので、何をする拡張かだけを書き、この文は入れない（2026-09-25）
- サイト側の検知を回避する機能や、規約をすり抜けるための機能は入れない（§21）
...
録音とスライドは録音中から拡張側のストレージ（OPFS）に逐次書き込む。サーバー未起動・処理失敗でも、あとから接続して再送すれば回収できる。
...
各コンテキストの責務は §6.2 を参照。利用者向けの全体図（Mermaid）は README の「しくみ」にある。
...
- `chrome.downloads` によるエクスポート（フォールバック兼 Phase 2 / 6 の動作確認手段）は 2026-09-24 に廃止した（§11.2）。
...
- サーバーが初回起動時にランダムトークンを生成して `~/.lec-scribe/token` に保存する。当初はユーザーが拡張の Options に 1 回貼り付けていたが、下の承認フローに置き換えたので、設定画面の貼り付け欄は外した（2026-09-18）。この共有トークンは curl での確認や開発に使うために残し、端末にもログにも出さない。
- サーバーは `Authorization: Bearer <token>` と `Origin: chrome-extension://…` を検査し、`127.0.0.1` のみで listen する。
- 貼り付けをなくす承認フロー（2026-09-09）: 未接続のときはサイドパネルに出る「このMacと接続」（設定画面にもある）を押すと、service worker が `POST /pair` を送り（当時あったポップアップが、ダイアログにフォーカスを取られて閉じるため）、サーバーが macOS のダイアログ（osascript）で「Chrome 拡張 xxx が接続を求めています」と確認し、「許可」なら拡張専用のトークンを発行して返す。拡張はそれを保存し、以後 Bearer で送る（承認済みの拡張 ID とトークンは `~/.lec-scribe/trusted.json`）。`/pair` は `Origin` が `chrome-extension://<id>` の要求だけ受け付けるので、Web ページや別の拡張はトークンを受け取れない（Origin はブラウザが付ける）。ただしブラウザ以外（curl など）は Origin を自由に書け、拡張 ID も秘密ではないので、承認済みの ID を名乗られてもダイアログなしではトークンを渡さない（2026-09-18）。
- トークンは承認 1 回ごとに 1 つ（2026-09-18）。サーバーには同じ ID の拡張（別の Chrome プロファイル、入れ直し）を見分けられないので、ID ごとに 1 つにして作り直すと、同じ ID のほかの拡張や、送信を始めたときのトークンで進み具合を見ている offscreen まで切れる。そのため:
  - 押し直し: 拡張は今のトークンを添えて `/pair` を送る。サーバーがまだ承認しているその ID のトークンなら、ダイアログを出さずに同じトークンを返す（新しく渡すものがないため）
  - トークンを持たない要求だけダイアログを出し、「許可」ごとに新しいトークンを足す。ほかの承認（同じ ID のものも）には触らない
  - ダイアログには、同じ ID が接続済みなら「この ID の拡張はすでに接続済みです（入れ直しや別プロファイルでなければ許可しない）」、別の ID が接続済みなら「すでに別の拡張が接続済みです: 名前（ID）」を足す。「LecScribe」を名乗る別物を、初めての接続と同じ見た目にしないため（人は 32 文字の ID を見比べない）
  - ダイアログの既定のボタン（Return で押される方）は「許可しない」。同じ Mac の別のプログラムもダイアログを出させられるため
  - 取り消すのは設定画面の「接続を解除」だけ。`POST /unpair` でそのトークンの承認だけを消し、拡張は保存したトークンを消す（サーバーに繋がらなくても拡張の側は解除する）
- トークンで認可するのは、host_permissions のある拡張ページからの GET には Origin が付かないため。
...
### D-12 UI は「アイコンでサイドパネルを開く → パネルで Start・監視」。状態は offscreen document と `chrome.storage.session` に持つ

- `chrome.tabCapture.getMediaStreamId` は「アイコンをクリックしたその時点のタブ」にだけ許可（`activeTab`）が出て、別のサイトへ移動すると失効する。開きっぱなしのサイドパネルから Start すると許可切れで失敗することを実機で確認した（2026-09-08）。
- 2026-09-21 までは、この制約のためにアイコン → ポップアップ（`sidepanel.html?mode=popup`）→ Start → サイドパネル、の 2 段にしていた（Start の直前に必ずアイコンを押すことになり、許可が新しい）。ポップアップは縦が限られて一覧が見づらく、「やり直す」だけしたいときにも不便だったので、次の形に変えた。
- **アイコンのクリックで、そのタブにサイドパネルを直接開く**（2026-09-21）。manifest にポップアップは置かず、service worker が `action.onClicked`（`activeTab` が出る正規のきっかけ）で `sidePanel.setOptions({ tabId, enabled: true })` と `sidePanel.open({ tabId })` を呼ぶ。`open` はユーザー操作の直後でないと呼べないので、`setOptions` の完了は待たない。パネルが開いているときにもう一度アイコンを押すと、許可だけが新しくなる。
- **許可が切れたらパネルを閉じる。** 開いたタブと、そのとき見ていたページのオリジンを `chrome.storage.session` に覚えておき、そのタブの読み込みが終わったとき（`tabs.onUpdated` の `complete`）に移動先のオリジンが違えば、許可が切れている（`activeTab` が残るのは同じオリジンの中を移動している間だけ）。何もしていないときに限って `setOptions({ tabId, enabled: false })` で閉じる。次に使うときは必ずアイコンを押すことになり、許可が新しくなる。同じサイトの中の移動では許可は残るので、閉じない。録音中・処理中・送信待ち・書き出し中は閉じない（録音は移動しても続くし、進み具合を見ているかもしれない）。「URL が読めるか」では判定しない（`http://127.0.0.1/*` は `host_permissions` で常に読めるので、許可が切れていても読めてしまう。2026-09-21）。
- それでも許可が切れたまま Start した場合（処理中に別のサイトへ移動し、終わったあとそのまま押した、など）は、失敗のエラー文がアイコンを押し直すよう案内する。
- 実機で確認済み（2026-09-21）: アイコンでパネルを開いてすぐ Start できる。同じサイトの別のページへ移動したあとも Start できる。処理中は別のサイトへ移動しても閉じず、処理が終わったあとの移動で閉じる。
- 正本は `chrome.storage.session` と offscreen document。service worker は 30 秒で停止しうる。


サイドパネルはアイコンを押したタブにだけ出す（2026-09-09。当時は録音を始めたタブ）。アイコンを押したときに `sidePanel.setOptions({ tabId, enabled: true })` でそのタブ向けに有効化し、全タブ共通のパネルは起動時に `setOptions({ enabled: false })` で無効にしておく。別のタブでは画面を広く使え、録音の様子を見たければ、そのタブでもアイコンを押してパネルを開く。
...
- permission は Phase ごとに必要になった時点で追加する（現在: `tabCapture` / `offscreen` / `activeTab` / `storage` / `sidePanel` / `scripting` / `alarms`。`downloads` は書き出しの廃止とともに 2026-09-24 に外した（§11.2）。`alarms` は送信に失敗した行列を時間を置いて送り直すため、2026-09-11 に追加した（§6.5、#8）。host_permissions は `http://127.0.0.1/*` のみで、ローカルサーバーと fixture ページへの注入テストに使う）。

### 6.2 コンテキストと責務

| コンテキスト | 責務 | 寿命・注意 |
|---|---|---|
| side panel | Stop、状態表示、セッション一覧（フォルダを開く / やり直す / 文字起こしする / 非表示 / 削除 / 中止。状態ごとに出し分ける。§15.1）。Start もここで押す（アイコンのクリックで service worker がそのタブに開く。D-12） | ページ操作で閉じない。`storage.session` の変更を購読して描画する |
...
```ts
// パネル → sw
{ type: 'START', tabId } | { type: 'STOP' } | { type: 'RETRY_UPLOAD' } | { type: 'DISCARD' }
...
{ type: 'CAPTURE_STOP' }
{ type: 'UPLOAD', sessionId } | { type: 'SESSION_FILES', sessionId } | { type: 'DISCARD', sessionId }
...
`chrome.downloads.download()` で `~/Downloads/LecScribe/<sessionId>/` に書き出す機能は、2026-09-09 に UI から外し（サーバーで処理する運用では音声も `~/LecScribe/<セッション>/.lecscribe/` に残るため）、2026-09-24 に service worker 側の実装と `downloads` 権限ごと廃止した。ストア公開にあたり、使っていない権限（インストール時に「ダウンロードを管理する」と警告される）を要求しないため。サーバーに一度も送っていない録音は OPFS に残り、あとから接続して「文字起こしする」を押せば送れる。

offscreen 側には、セッションのファイルを blob: URL で読み出す `SESSION_FILES`（旧 `EXPORT`）を残している。スモークテストが OPFS の中身（`audio.webm`、`slides.json`、`timeline.json`）を確かめるための入り口で、利用者向けの機能ではない。

### 11.3 削除・中止・非表示

一覧の操作は 3 つに分かれる（2026-09-11。それまでは「破棄」1 つで、何が消えるのか分かりにくかった）。確認はブラウザの `confirm()` ではなくパネル内の `<dialog>` で出す（サイドパネルでは `confirm()` が表示されないまま閉じられることがあったため）。
...
- **中止**（処理中・送信待ちだけ）: サーバーの処理を止める。初回の処理なら途中のフォルダと録音も消し、やり直しの中止なら処理だけ止めて前回の結果と録音を残す（`{ output: 'keep', keepRecording: true }`）。
  - **やり直しの中止は、やり直す前の状態に戻す**（2026-09-20）。`POST /finalize` は `pipeline.json` を `queued` で上書きする前に、完了していた状態の控え（`.lecscribe/pipeline.previous.json`）を取る。中止されたらそれを `pipeline.json` に書き戻すので、段階は `done` のまま、前回の `result` も残る（`notes.md` は前回のまま残っているので、これが実態に合う。以前は `cancelled` になり、完成したノートがあるのに状態は「中止」だった）。控えは完了・失敗で終わったら捨て、前回が完了でなかった（失敗・中止のまま）なら作らない（もっと前の完了状態に戻さないため）。控えが無いときは今までどおり `cancelled`
  - 文字起こしの記録（`transcript`）だけは、ディスクにある report と合うものにする。この回で whisperkit を動かしていなければ前の記録のまま、動かしている途中で中止されたら落とす（report を書き換えている最中なので、残すと壊れた report を再利用して失敗し続ける）、この回で作り直したあとなら新しい記録にする。書き込みの順序に頼らず、`Job.whisper`（untouched / running / finished）で決める
  - 作業ファイル（`lecture.md`、`scenes.json`、`transcript.*`）は、統合より後で中止すると今回の内容になっている。利用者が読む `notes.md` は前回のままで、作業ファイルは次の「やり直す」で揃うので、元には戻さない
  - **初回の処理でも、ノート作成中の「中止」は消さない**（2026-09-20）。文字起こしまで済んでいるので、ノート作成（LLM）だけを止め、文字起こしのままのノートで完了にする（録音も残る）。Codex が利用上限で進まないのを見て「中止」を押し、録音ごと失うのを防ぐ。確認ダイアログの文言もそう変える。拡張は `POST /cancel { finish: true }` を送るだけで、進み具合の監視は止めない（完了は監視がいつもどおり受け取る）。サーバーがもうノート作成中でなければ何も起きない（`finished: false`）。サーバーは処理全体を止める signal とは別に LLM だけを止める signal を持つ（前者で後者も止まる）
...
- 文字起こしはできたがノートを整えられなかった行には、注意書きを 1 行出す（§13.5、2026-09-20）。拡張は処理が `done` になったとき、サーバーの `result.notes` が `false`（全体）か、`true` で `notesError` がある（一部）かを `status.json` の `notesProblem`（`failed` / `partial`）と `notesError` に残し、「やり直す」で整えられたら消す。処理中・送信待ちの間と「データなし」の行には出さない。印は完了を見届けた offscreen が書くので、見届けられなかったとき（途中でブラウザを閉じた、service worker が止まった、など）は古いまま残り、やり直して整ったあとも注意書きが出続けていた。そこで一覧は、処理済みの行についてサーバーに聞くとき（フォルダの有無を確かめる `GET /sessions/:id/status`。もともと行ごとに 1 回呼んでいる）に結果も読み、保存してある印と食い違っていたら書き直して描き直す（2026-09-20）。信じるのはサーバーが `done` のときだけ。やり直しの途中（`queued`〜`polishing`）や、失敗・中止のままの状態には結果が無く、そこで印を消すと、まだ整っていないのに注意書きが消えるため。処理中・送信待ちの行は offscreen が書くので触らない。サーバーに繋がらないあいだは、保存してある印で出す
...
- サーバーは **約束の版** `API_VERSION`（`server/src/app.ts`、整数）を `/health` の `api` で返す。上げるのは HTTP の約束が変わったときだけ（拡張が送る・受け取る項目が増える、意味が変わる）。内部の改善やノートの作り方の変更では上げない。`1` = 2026-09-11（cancel の `force`、status の 404、title 先頭のフォルダ名まで）。`2` = 2026-09-11（`/health` に `vision` と `visionReason`。#17。拡張が必要とする最低の版は 1 のまま。見せるだけの項目なので、拡張は項目が無ければその行を出さないだけで、古さの判定には使わない。項目の有無で「古い」と決めないのが §12.1c の規則で、表示の有無を項目で決めるのは別）。`3` = 2026-09-18（`/pair` が添えられたトークンを見る、`POST /unpair`。D-10。最低の版は 1 のまま。古いサーバーでも押し直しは従来どおり同じトークンが返り、解除は拡張の側だけで済む）。`4` = 2026-09-20（cancel の `finish` と、status の `result.notesCancelled`。§11.3。最低の版は 1 のまま。古いサーバーは `finish` を知らず、ただの中止として扱う。録音は拡張に残るので送り直せる）
- 拡張は必要な最低の版 `REQUIRED_SERVER_API`（`extension/src/health.ts`）を持ち、パネルを開いたとき（接続済みなら）と設定画面の「接続テスト」で `/health` を読んで比べる。`api` を返さないサーバー（この仕組みより前の版）は 0 とみなす。古ければ警告の欄に「Mac 側のサーバーが古く（版 N。この拡張には版 M が必要）、一部の機能が動きません。次にログインしたとき自動で更新されます。すぐに直すには … update.sh」と出す。送信は止めない（録音した分の文字起こしはできるため）
...
### 12.1d ポートをほかのアプリが使っているとき（2026-09-18）

ポートは 47321 に固定し、設定画面に欄を置かない。macOS が空いているポートを自動で割り当てる範囲（49152〜65535）の外で、既知のサービスの一覧にもないので、偶然重なることはまずない。それでも、この範囲のポートを決め打ちで使うアプリはある。重なったときに利用者にできるのは相手のアプリを止めるか、相手のポートを変えること（相手はたいてい自分で起動した開発用のサーバーで、ポートを選び直せる）なので、それを分かるように伝える。

- **サーバー**: 待ち受けが `EADDRINUSE` なら、1 秒後に 1 回だけ黙って試し直す（再起動の直後に前のプロセスがまだ放していないだけのことがある）。それでも駄目なら、相手の名前（`lsof`。別のユーザーのプロセスは見えないので、そのときは「ほかのアプリ」）を入れた「ポート 47321 を「〇〇」が使っているため、LecScribe のサーバーを起動できません。〇〇を終了するか、〇〇のポートを変えてください。」をログに書き、10 秒ごとに試し直す。相手が終われば何もしなくても待ち受けを始める。終了して launchd に起動し直させないのは、そのたびに自動更新の git fetch が走り、ログにも同じ失敗が並ぶため。知らせは相手が変わったときだけ書く
- **ターミナル**: `agent.mjs install` / `restart` は、起動を待つ間にこの知らせがログに出たら、それを表示して失敗で終わる（サーバーは試し直しを続ける）。`status` は、応答がなく、最後に起動したあとにこの知らせが出ていれば表示する
- **拡張**: `/health` の応答が LecScribe のもの（JSON で `ok` が true / false）でなければ、「ポート 47321 をほかのアプリが使っているため、…そのアプリを終了するか、そのアプリのポートを変えてください。」を、未接続の画面、パネルの警告、設定画面の「接続テスト」に出す（相手の名前は拡張からは分からない）。送信中にほかのアプリが応答したら、同じ文で送り直せる失敗にする
- トークンを送る前に相手を確かめることはしない。悪意のあるプログラムなら LecScribe の応答をまねられるので防げず、そうでないアプリはトークンを使わないため

### 12.2 API

| メソッド | パス | 内容 |
|---|---|---|
| GET | `/health` | `{ ok, version, api, commit, ffmpeg, whisperkit, vision, model, llm, authorized, paired, processing }`。`vision` は見た目の判定の補助コマンドの状態（api 2 から、#17）: `ready`（使える）/ `building`（作っている途中）/ `idle`（まだ作っていない、または置き場から消えた。次に使うとき作る）/ `failed`（作れなかった。`visionReason` に理由）。Vision を使わない設定と macOS 以外は `null`。ディスクを見るだけでビルドは始めない（CLT が無い Mac にも `/usr/bin/swiftc` の shim があり、`/health` のたびに叩くとダイアログが出る）。ソースのハッシュはプロセスごとに 1 回だけ計算する（`/health` が毎回ファイルを読まない。ソースが変わる更新は必ず再起動を伴う）。`api` は拡張との約束の版（§12.1c）、`commit` は動いているコードの短い SHA（診断用。分からなければ null）。ffmpeg / whisperkit の有無は `resolveBin` が覚えている（1 分で忘れる。毎回 PATH を歩いていた #9） |
...
| POST | `/pair` | `{ name? }`。認証不要だが `Origin: chrome-extension://<id>` 必須。macOS のダイアログで承認されると `{ paired: true, token }`。そのたびに新しいトークンを足し、ほかの承認には触らない。その ID の承認済みトークンを `Authorization` に添えた押し直しは、ダイアログなしで `{ paired: true, already: true, token }`（同じトークン）。拒否は 403、ダイアログ表示中は 429（D-10、2026-09-18） |
| POST | `/unpair` | 認証必須。送ってきたトークンの承認だけを取り消して `{ ok: true, removed: true }`。共有トークンなら何も消さず `removed: false`（設定画面の「接続を解除」。api 3、2026-09-18） |
| POST | `/sessions/:id/cancel` | `{ delete?: boolean; force?: boolean; finish?: boolean }`。`finish` はノート作成だけを止めて完了にする（ノート作成中でなければ何もしない。応答に `finished`。api 4、§11.3）。それ以外は: 待機中なら取り下げ、実行中なら子プロセス（ffmpeg / whisperkit / codex）を止める。`delete` でフォルダごと削除（`notes.md` があるフォルダは残す）。`force` で `notes.md` があっても削除（一覧の「削除」。2026-09-11） |
...
- まず、**ほぼ一色の画像**は載せない（`reason: 'blank'`、2026-09-22）。画素の 99.5% 以上が、その画像の中央値の色から 24 以内（`uniformFraction`）。真っ黒（「はじめに」の 025。フェードの途中で撮れた）、動画の先頭の灰色 1 枚（13 章 GD の 001）、暗い画面に小さな点だけが動くコマ（7 章 GD の 060、99.7%）が該当する（060 が外れた結果、それまで 057 と「中身が同じ」でまとまっていた別の字幕の 059 が載るようになる）。白地に短い見出しだけのスライドは 28 セッションで最大 97.9%、読める文字のある暗い画面は 98.0% なので残る。160×90 では残り 0.5% が 72 画素しかなく、細い罫線だけの区切りや淡い文字のタイトルも掛かりうるので、**文字認識が 1 文字でも読めた画像は一色とみなさない**（本当の真っ黒からは何も読めない。2026-09-24）。比較の相手にもしない: まとまりの基準にも、直前の画像との比較にも、「この画面が続いた時間」の区切りにも使わない（真っ黒と「暗い背景に字幕だけ」は画素差 3% で「中身が同じ」になってしまうし、フェードのコマで区切ると長い戻りが短い戻りに見える。2026-09-24 のレビューで直した）。`sameSceneAs` には今載っている画像を入れる（間の発話はその下に入る。`scenes.json` から辿れるように）。最後の 1 枚を載せる設定でも、まとまりの代わりには選ばれない。
- 次に、最後に載せた画像と画素が 5% しか違わない画像は、映像でもスライドでも外す（`reason: 'identical'`。160×90 の 5% は 720 画素で、見た目にはほぼ区別がつかない。当初 2% だったが 5 章で 2〜4% の組が残ったので上げた）。拡張側の取りこぼしの受け皿（4 章の録音で、最終状態の上書きと次のキャプチャが同じフレームになり、まったく同じ画像が 2 枚並んだ）。
- 次に、**見た目の距離**（macOS の Vision、`VNGenerateImageFeaturePrintRequest`。2026-09-10）で比べる。画素や色の分布では分けられなかった「同じ写真の続き」「メニューを開いただけ」「少しスクロールしただけ」を、人の目に近い基準で拾う。Swift の小さな補助コマンド（`server/tools/imagefp.swift`）を `swiftc` でビルドして `~/.lec-scribe/bin/` に置き（常駐サーバーは起動時に作る。§12.1b。間に合わなければ最初の処理で待つ）、画像パスを渡して距離の行列を受け取る。Mac の中だけで動きトークンを使わない。`swiftc`（Xcode Command Line Tools）がなければこの判定を飛ばす。ビルドは 3 分で打ち切る（CLT が無い Mac にも `/usr/bin/swiftc` があり、「開発者ツールをインストールしますか」のダイアログを出したまま返らないことがある。打ち切らないと、あとから補助コマンドを待つ文字起こしも一緒に止まる）。**作れなかった結果は覚えない**（次に呼ばれたらやり直す。常駐サーバーは何日も動くので、起動時にたまたま駄目だっただけで、その間ずっと諦めることになる）。**覚えたパスはまだあるか確かめてから使う**（置き場を退避・削除されたら次の文字起こしで作り直す。/health の見え方と食い違わないように）。状態は `/health` の `vision` で分かり（§12.2）、拡張の設定画面の「接続テスト」と `agent.mjs install` / `status` に「見た目の判定（Vision）: あり / 準備中 / まだ作っていません / 作れませんでした（理由）」と出る（2026-09-11、#17。`install.sh` が前もってビルドしていたときはターミナルに結果が出ていたが、起動時に移して見えなくなったため）。CLT の導入を勧めるのは「作れませんでした」のときだけ。まだ作っていない・作っている途中を「なし」と一括りにすると、CLT がある Mac に導入を勧めてしまう。
  ...
  - 両方が写真・映像（色の分布の多様さが 3.0 ビット以上。文字中心のスライドは 0〜3）なら、`--scene-vision-photo`（既定 0.55）以下でも同じ場面とみなす。5 章で「ほぼ同じ」と指定された 17 組は 0.10〜0.70、指定されなかった組は 0.59 以上（3 章では同じ場面 0.15〜0.98、別の場面 0.75 以上）。6 章で「カメラ本体」「粗微動ユニット」とラベルの付いた別の装置の写真が 0.53 で「同じ場面」にまとまり、説明ラベルが全部出た 1 枚が消えたので、いったん 0.45 に下げたが、8 章で同じ場面の続き（枝を揺すって傘に落とす映像の寄り引き 0.535、講師が資料を持ったまま顔を上げた 0.501）が漏れた。そこで 0.55 に戻し、代わりに **両方に 4 文字以上の文字があって中身が違う（そろい具合 0.8 未満で、一方が他方に含まれもしない）なら写真同士の判定ではまとめない** 歯止めを付けた（文字認識が使えるようになったため。6 章の 0.53 はこれで残る）。まとめ損ねは重複が残るだけだが、まとめ過ぎは中身が消えるので、歯止めは安全側に効かせる。副作用として、同じ映像でも字幕が変わると別の画像として残る（6 章で 3 組）。字幕が変わる＝新しい情報なので許容する。ただし、まとまりに加えた画像の文字が基準の文字と食い違っていたら（同じ板書が BAsceT / EAsckET / 54SsceT と読まれる手書きなど、読み取りが安定しない場面）、その場面ではこの「別のラベル」の拒否をしない（2026-09-17。6 章（グラフィックデザイン）で講師の動きだけが違う 3 枚が別々に残っていた。読み取りの信頼度は印刷文字でも 0.5 になることが多く区別に使えず、そろい具合の下限で緩める案は「カメラ本体」と「カメラ台座」のように先頭だけ同じ別ラベルを拒否できなくなるので取らなかった。26 セッションで再判定し、新しくまとまるのはこの 3 枚と 15 章の Illustrator のメニュー開閉だけ）。 撮影した紙面の上で手が動く画面（下の「手が動いただけの組」の帯）では、この規則は使わない（2026-09-24）。
  ...
  - **画面を少しスクロール・パンしただけの組**も外す（`reason: 'panned'`、2026-09-20）。アプリの操作画面（Illustrator のキャンバスをずらした）や Web ページ（少しスクロールした）は、中身が同じでも画素の 2〜3 割が変わり、Vision の距離も 0.21〜0.31 で「ごく近い」（0.2）をわずかに超える。細かい UI の文字は読み取りが安定せず（同じメニューが「Hllustrator」「四珍」などになる）、文字の規則にも掛からない。Vision の閾値を上げると、同じ型で本文が違うスライド（0.23〜0.27）がまとまるので、代わりに「違っている画素の大半が、同じ向きの平行移動で説明できる」ことを見る（`panResidual`）。
    - 160×90 のサムネイルで、横 ±24・縦 ±12 画素（画面の 15%・13%）の移動を総当たりする。違っている画素 p について a(p) = b(p+d) かつ b(p) = a(p−d) なら「説明できた」（片方だけだと、文字が白地に重なるだけで説明できたことになる）。縮小で 1 画素未満のずれが出るので、移動先の周り 1 画素のどれかに合えばよい。ツールバーなど動かない部分は、もともと違っていないので数えない
    - 条件は 3 つ: Vision の距離が 0.35 以下（`--scene-vision-photo` を下げていたらそちらが上限。0 にすればこの規則も効かない）、説明できずに残った画素が違っている画素の 4 割以下、説明できた画素が全体の 1 割以上。27 セッション 1,044 枚で、まとまるのはパン・スクロールの 4 組だけ（11 章 GD の 122→123 は残り 0.36、10 章 GD の 028→029 は 0.07、GD I-2 の Web ページ 054→055 は 0.09、056→057 は 0.22）。本文が違うスライドは 0.43 以上、図形を描き直した画面は 0.5 以上。少ししか動いていないのに中身が変わった組（10 章で腕の向きを変えた 039→040）は、説明できた画素が 4% しかなく残る
    - 基準の画像（まとまりの最初の 1 枚）とだけ比べる。直前の画像とも比べると、長いページを少しずつスクロールした全部が 1 枚にまとまり、最後の画面しか残らない。基準とだけなら、動いた量が探す範囲を超えたところで次の 1 枚が残る
    - 計算は、ほかの規則で決まらず Vision が上の上限以下で画素が 1 割以上違う組だけ。27 セッション合計で約 11 秒増える（1 セッション平均 0.4 秒）
  - **画面を大きくスクロールしただけの組**も外す（`reason: 'scrolled'`、2026-09-22）。上の平行移動の規則は縦 12 画素（13%）までしか探さない。Web ページをカード 1 行分（25 画素前後）以上スクロールすると探索の外になるうえ、写真を含む画面は縮小で画素がぴったり合わず、探索を縦 40 画素・320×180 まで広げても「説明できた画素」は 3〜4 割にしか届かなかった（GD I-2 の 2 章 057→058→059）。そこで画素ではなく **行ごとの平均色の並び** を見る（`rowScroll`）。
    - 160×90 のサムネイルで各行の平均 RGB を出し、片方を縦に ±45 行（高さの半分。それ以上は重なりが薄い）ずらして重ね、重なった行のうち平均色の差が 16 以内の行の割合（差に応じて 0〜1）が最も高いずれを探す（「地の色（行の中央値）から離れた行だけを数える」形も試したが、カードや写真が並ぶページでは中央値が地の色にならず、2 章の 057→058 のような本物のスクロールが 0.5 を切ったので、重なった行は全部数える）。8 行未満のずれは見ない（小さな移動は平行移動の規則の担当。カーソルが動いた・図が白黒になっただけの画面（12 章 GD の 005→006）は 4 行前後のずれで重なってしまう）
    - 条件: 一致が 0.6 以上（2 章の 057→058 は 0.65、058→059 は 0.76、5 章 GD の 028→030 は 0.95。別の作品は 0.06〜0.34）、Vision の距離が 0.4 以下（スクロールした組は 0.24〜0.34。図が増えた・白黒になった別の画面は 0.46 以上。`--scene-vision-photo` を下げていたらそちらが上限）、画素が 1 割以上違う、行の並びに凹凸がある（中央値から 16 以上離れた行が 3 割以上）か、凹凸が少なければ**中身のある行だけで重ねる**（下）、両方に 3 行以上の文字があるなら共通する行が 3 割以上（`sharedLineRatio`。読み違いを許して行ごとに 7 割合えば同じ行。同じ配色の別のページを除く。10 章 GD の 070→073 は 0.2）
    - 行の平均色は横の並びを見ないので、周期的なリストや表では中身の並びが違うページでも行だけは重なる（10 行周期のリストで並び順だけ違う 2 枚が一致 1.0 になった。2026-09-24 のレビュー）。そこで見つかったずれで重ねたとき、**列ごとの平均色**も 0.6 以上合うことを求める（`columnMatch`。数えるのは行の平均色が合っている行だけ。動かないヘッダーが相手の本文と重なる行を混ぜると列の平均が全部ずれるため）。列は横に ±40 画素ずらして探す（斜めに送った画面。Illustrator のキャンバスをドラッグした 11 章 GD の 023〜028）。数えるのは、どちらかが地の色（列の中央値）から離れている列だけ（横にずらすと中身のある列が重なりの外に出て、余白同士だけで「合った」ことになるため）。列に凹凸がなく判断できない画面（横いっぱいの帯や本文の行だけ）では列は見ず、行と文字に任せる。同じ一致なら小さいずれを取り、重ねられる行がなければ `scrollShift` は残さない
    - **凹凸の少ないページ**（白地に小さなロゴや数行の文字だけの Web ページ。中央値から離れた行が 3 割未満）では、全部の行で測ると余白の白同士がどんなずれでも合ってしまうので、**地の色（行の中央値）から離れた行だけ**を数え、6 行以上重なって一致が **0.75 以上**なら同じ画面とみなす（2026-09-24）。GD I-2 の 3 章で、白地のポートフォリオサイトを少し送った 023→024 は、動いた画素が全体の 6% しかなく平行移動の規則（説明できた画素 1 割以上、残り 4 割以下）に届かず、Vision も 0.232 で「ごく近い」（0.2）をわずかに超え、どの規則にも掛からず 3 枚に割れていた。中身のある 9 行は 0.86 で重なる。別のスライド（同じ型で本文が違う、色→白黒、白地の見出しだけ）は 0.04〜0.52、10 章 GD の図形の塗りを線に入れ替えた別の状態は半画面ずらした位置で 0.64 なので、線は 0.75。この経路では列の確認をしない（列の平均も余白に埋もれ、端から出入りする中身に引きずられる）。画素の差の下限（凹凸のあるページの 1 割）も置かない（白地のページを送った組は 6〜10% しか変わらず、5% 以下の「中身が同じ」は規則 1 が先に外す）。数える行が少ないぶん、見出し 1 本の高さが違うだけの別のスライドも重なるので、**両方に 4 文字以上の本物のラベルがあるときだけ**使う（文字が読めない・短すぎる組は残す。2026-09-25 のレビュー）。48 セッション 2,169 枚で新しくまとまるのは 3 章の 023〜025 と、11 章 GD I-2 の Web ページを送った 2 組だけ
    - 文字の歯止めは 2 段。本文が 3 行以上あれば共通する行で見る（下）。それより少ない（ラベル・字幕）なら、写真同士の規則と同じ「別のラベル」の歯止め（両方に 4 文字以上あってそろわず、含まれもしない）を掛けるが、**雑音を除いた本物のラベルだけで**見る（`labelText`。数字と記号ばかりの行＝アプリの時計「1月20日（月）16:42」やファイル名の断片、3 文字未満の断片＝ピクトグラムの「山」「炭」「35S」の誤読、を除く）。雑音まで数えると、同じ画面を送っただけの 5 章 GD のピクトグラムの一覧（028→030）と 11 章 GD のキャンバス（023〜028）が「別のラベル」で止まる。写真同士・同じショットの規則は雑音を除かない（除くと、同じ Illustrator で別の作品を開いた画面まで写真同士の規則でまとまった。時計やファイル名が違うことが、たまたま別の画面の根拠になっている）。行の一致・列の一致・文字の順に見る（文字の比較が最も重いので最後）
    - 平行移動と同じく基準の画像とだけ比べるので、長いページを少しずつ送り続けると、探す範囲を超えたところで次の 1 枚が残る（2 章では 056〜058 が 058 に、059 は残る）。28 セッション 1,069 枚で新しくまとまるのは 2 章の 057、5 章 GD の 028（ピクトグラムの一覧を送った。拡大率も少し違うが行と列の並びは重なる）、11 章 GD の 023〜027（Illustrator のキャンバスを斜めに大きく送りながら描いた）だけ。2 章の 053→055 のように画面の半分以上が入れ替わるスクロールは重なりが 4 割しかなく、まとめない
  - **撮影した紙面の上で手（指）が動いただけの組**も外す（`reason: 'hand'`、2026-09-24）。GD I-2 の 4 章は、本のページをカメラで撮って講師が指さしながら話す動画で、指が動くたびに拡張が保存し、95 枚のうち 12 組が指の位置だけの違いだった。同じページに手が入っただけの組は画素の 7〜23% が変わり Vision は 0.21〜0.36、ページをめくった組は Vision 0.35〜0.72 で画素の 15〜57% が変わる。Vision だけでは重なるが、読み取れたラベルの行は、同じページなら 0.33〜1 が共通し、別のページは 1 行も共通しない
    - 入口は、拡張が記録した `trigger.stillFraction` が **半分以上 0.976 未満**（何かが動き続けているが映像ではない画面。スライドや Illustrator の画面は 0.976 以上、映像は半分未満）で、自動保存の `trigger.diffPrev` が 0.3 未満（手が動いた程度: 0.03〜0.18。被写体が大きく動いた映像、14 章 自然の蝶は 0.36〜0.49）の画像。基準の画像も静止部分が同じ帯なら（めくった瞬間の画像は変化が大きいが、そのあと手が動いた画像の基準になる: 079→082）、撮影された紙面として比べる。この画像の側の帯を見るのは、雑音で静止部分が 0.976 に届かないスライド（12 章 自然の 008、0.972）を基準に、同じ型で本文だけ違う次のスライドを吸わないため
    - 条件: Vision の距離 0.4 以下、画素の差 25% 以下、両方にラベルらしい行（`labelText` が残す行）があれば共通する行が 2 割以上（手で隠れる行が変わるので低め。別のページは 0）、そのうえで色の分布の一致 0.75 以上。少ない方に 4 行以上あるなら共通する行は 2 行以上要る（各ページ共通の柱・フッター 1 行の一致を根拠にしない。同じページに手が入った組は 8 行中 2〜4 行が共通する）。行の一致は編集距離 0.7 のほか、短い方が丸ごと含まれるか、**5 文字以上・短い方の 4 割以上が続けて同じ**で両方の行末で終わっていないこと（同じ印刷物の同じ行は、手や画面の端で切れる位置と読み違いが毎回違う: 「3サクウブチケア（コンタクルレンスタ」と「サクラブチケア（コンタク▶レンズ量28）…」。行末だけの一致は「することができる」のような決まり文句で終わる別の文でも起きる）。基準の画像とだけ比べるので、基準で手に隠れていた行は読めておらず、3 行未満でも 1 行ずつで測る。ラベルらしい行がどちらかにまったくなければ、生の文字が半分そろっている（本文の縦組みは読み違いだらけでラベル行が残らないが、同じページなら 0.5 以上そろう: 13 章 GD I-2 の 071 は 0.56）か、別のラベルが付いていないこと（時計しか読めなかった基準に、本文が読めた別のページを吸わない）と、色の分布 0.85 以上で見る。画素の差は 25% までだが、ラベルが共通するなら 35% まで（手が大きく動いた 13 章の 122→123 は 29%）。文字で同じと言えた組だけ色の分布を測る（2026-09-25 のレビューで整理）
    - この帯の画像同士では、写真同士の規則（距離 0.55 まで）は使わない。同じ本の別のページが 0.35〜0.5 で、手が動いただけの組と重なるうえ、手が入ると文字の読み取りが毎回変わって「読み取りが安定しない」扱いになり、別のラベルの歯止めが外れて別のページまでまとまっていた（4 章の 011〜022 の 6 見開きが 023 の 1 枚に、028〜032 が 033 に）。同じショットの規則（6）でも、この帯では歯止めを外さない
    - 帯の穴: 講師のワイプや時計が常に動くスライド講義も静止部分 0.90〜0.95 に入りうる。そこでは同じ型のスライドが柱を共有するので、ラベルが 3 行以下のスライド同士なら柱 1 行の一致で手が動いただけとみなす穴が残る（手元の 48 セッションにそういう講義はない。出てきたら、その講義で線を引き直す）
    - 4 章 GD I-2 は 51 → 48 枚。指の位置だけ違う 12 組がまとまり、写真同士の規則で消えていた 9 枚（006・007・012・014・015・020・022・029・030）が戻る。同じく本を撮った 13 章 GD I-2 は 69 → 82 枚: 写真同士の規則が別の見開き 10 枚（017・019・024・061・080・093・110・119・121・133）を隣のページにまとめていたのが戻り、同じ見開きで手が動いた組のうち文字がほとんど読めないもの（036、146〜150）が 4〜5 枚残る。文字のない写真のページ（096）は色の分布の規則で次のページに吸われる（main は 093・094 を失っていた。台の上の本はどのページも色の分布が似るので、色の線を上げると 14 章 GD I-2 が崩れ、ここは残した）。48 セッション 2,169 枚でほかに変わるのは 3 章と 11 章の上の組だけ
  - **前の画面に短く戻っただけの画像**は載せない（`reason: 'revisit'`、2026-09-20）。講師が A → B → A → B と行き来すると、切り替わりは全部本物なので 4 枚とも保存され、比べる相手は最後に載せた画像だけなので、同じ 2 枚が 2 回並んでいた（GD I-2 の 1 章 023〜026）。
    - 基準と同じでない画像が、1 つ前のまとまりの画面（基準か、最後に加えた画像）と文字を使わない規則（中身が同じ・距離 0.2 以下）で同じで、その画面が続いた時間が **10 秒未満**（動画の時刻で。`REVISIT_MAX_SECONDS`）なら外す。間の発話は、今載っている画像の下に入る。外したあとに元の画面へ進めば、今の規則（中身が同じ）でまとまるので、A・B の 2 枚になる
    - 10 秒の根拠: 27 講義で「2 つ前と同じ画面に戻った」64 件を見ると、9.5 秒以下が 31 件、その次は 11 秒以上。9.5 秒以下の間の発話は長くて 90 字（1〜2 文）で、前の画像の下に入っても困らない。11 秒を超えると、その画面の説明が始まっている。戻って 90 秒、120 秒話している例もあり、長い戻りは画像ごと残す
    - 続いた時間は、次に「少しでも違う画面」（文字を使わない規則で同じでない画像）が撮られるまで。戻った先で操作して画面が変わったら、そこからは新しい画面として載せる。最後まで戻ったまま、または時刻が巻き戻っている（シークした）ときは長さが分からないので載せる
    - 戻っている間に撮れた続きの画像も外すが、**そのつど戻った先の画面と比べ直す**。直前の 1 枚とだけ比べて連ねると、画面が少しずつ変わっていく間ずっと外れ続け、戻った先とは似ていない画像まで外れて、その区間に載る画像が 1 枚も無くなる
    - 続けて外すのは、**つながった戻りの合計が 10 秒未満の間だけ**。1 枚ずつの「この画面が続いた時間」だけで決めると、戻った先で注釈を書く・少しスクロールするなどして 10 秒未満ごとに画面が変わる長い戻りが丸ごと外れ、その区間に載る画像が 1 枚も無くなる。合計が 10 秒を超えたところからは新しい画面として載せる（時刻が巻き戻っていたら、そこで戻りを打ち切る）
    - 戻っただけの画像は今のまとまりの画面ではないので、まとまりの最後の 1 枚（`--scene-keep last`）には選ばない。あとでもう一度、今度は長く戻った画像を、前の短い戻りと同じだからと外すこともしない
    - 27 講義 1,044 枚で、短い戻りは 34 件、載せる画像は 771 → 728 枚
  - 比較の相手は「最後に載せた画像」（まとまりの最初の 1 枚）だが、場面が少しずつ変わっていくと最初の 1 枚からは離れてしまう。そこで最初の 1 枚と同じでなければ、**直前の（外した）画像** とも比べ、間違えにくいルール（中身が同じ・距離 0.2 以下・文字が同じ・途中の状態）で同じなら外す（`via` に直前の画像を残す）。写真同士の広い判定と色の分布は直前との比較には使わない（少しずつ違う写真が連鎖して別の場面までまとまるのを防ぐ）。さらに前の（外した）画像とも比べるが、こちらは文字を使わない規則（中身が同じ・距離 0.2 以下）だけ（2026-09-17。講師が動く画面で、2 つ前の画像とは 0.145 なのに直前とは 0.27 だった。文字の規則まで広げると、見出しやフッターが共通の同じ型の別スライドがまとまる。15 章（グラフィックデザイン）の「仮想アドバイス」4 枚で確認）。`via` にはどの画像と同じだったかを残す。3・5・6・7・8 章の指定された組で確かめ（scratch の Python で全組み合わせの距離・画素差・文字を出して判定を再現）、まとめ過ぎはゼロ、拾えなかったのは 3 章の映像 8 組（構図が大きく変わる。距離 0.63 以上）と 5 章の 2 組（0.64 以上）、6 章の字幕が変わった 3 組だけ。
  ...
  - 画素・色・Vision・文字のどれで外したかは `scenes.json` の `reason`（`blank` / `identical` / `vision` / `text` / `grown` / `panned` / `scrolled` / `hand` / `same-scene` / `revisit`）に残し、比べた画像との文字のそろい具合を `textSim` に残す。平行移動を調べた組には、説明できずに残った画素の割合 `panLeft` と、見つかった移動 `panShift` も残す。縦のスクロールを調べた組には、比べた画像を何行ずらすと重なるか `scrollShift` とそのときの一致 `scrollMatch`、両方に 3 行以上の文字があった組には共通する行の割合 `sharedLines` も残す。
...
- 次に、`slides.json` の画像を順に見て、最後に載せた画像と **色の分布（RGB 各 8 段階のヒストグラム交差、§9.1d と同じ計算）** が `--scene-color`（`LEC_SCRIBE_SCENE_COLOR`、既定 0.65）以上そろっていれば載せない。対象は、静止部分が半分未満の画像（映像中心の画面）と、**写真・映像らしい画面（色の多様さ 3 ビット以上）で、切り替えを検知した瞬間の変化（`trigger.diffPrev`）が 0.3 未満だった画像**（2026-09-22。拡張の `cutThreshold` と同じ線。カットではなく、同じショットの中でカメラや被写体が動いた・字幕が出た、ということ。ゆっくり動くカメラが静物を写していると静止部分が 0.5〜0.8 と多く測られ、「はじめに」（自然を観る）の 014〜016（描く手元に字幕が出る。Vision の距離は 0.65〜0.84 で写真同士の判定にも届かない）が 3 枚とも残っていた。両方に 4 文字以上の文字があって中身が違えば別の場面（写真同士の規則と同じ歯止め。手書きの板書で読み取りが安定しない場面では根拠にしない）。**自動で保存した画像（`reason: change`）だけ**が対象。手動・開始時の保存は切り替えの瞬間の値を持たず、安定後の値（≒ 0）が `diffPrev` に入るので、利用者がわざわざ撮った写真が色で消えないように見ない（2026-09-24 のレビューで直した）。0.3 は 28 セッションの谷から取った値で、まとめたい組は 0.15〜0.27、残したい組は 0.45 以上。拡張の `cutThreshold` も 0.3 だが、あちらが効くのは映像らしい画面だけで、スライドと測られた画面の保存の線は `changeThreshold`（0.025）なので、0.3 未満＝ごく小さな変化ではない（だから写真らしさと自動保存の条件も要る）。28 セッションで新しくまとまるのは、この 3 枚と 005→006、9 章（自然を観る）の 009（組み立てた標本 → 開いた標本）、6 章 GD の対談の 6 枚（スケッチやサインのボードを見せる 075・087・093 を含む）。写真が切り替わる組（14 章の「カタバミ」の別の株 026→027、6 章のサインの写真 045→046・056→057）は変化が 0.45〜0.94 でカットなので残る。色の多様さを条件にしないと、白地のスライドはカットでも画素の変化が小さく、同じ型の別スライドがまとまる）。載せなかった画像の発話は、載せた画像の節に続けて入る（`assignSlides` に載せる画像だけを渡すことで自然にそうなる）。
- サムネイルは `ffmpeg -vf scale=160:90 -f rawvideo -pix_fmt rgba` で作る（1 枚あたり数十 ms）。取れなければ何も外さない。
- 対象は、**拡張が記録した `trigger.stillFraction` が半分未満の画像（映像中心の画面）** と、上に書いた **同じショットの続き**（写真らしい画面で、自動の保存の `trigger.diffPrev` が 0.3 未満）だけ。スライド中心の画面では、同じ配色で文字だけ違うスライドが「同じ場面」に見えてしまう（13〜15 章の録音で試すと、隣り合うスライドの 9 割が色の一致 0.65 以上だった）。記録のない古いセッションでは色では何も外さない。
- 画像そのものは `slides/` に残す。判断は `.lecscribe/scenes.json`（画像ごとに `shown` / `sameSceneAs` / `standsFor` / `reason` / `via` / `vision` / `pixelDiff` / `textSim` / `colorMatch` / `panLeft` / `panShift` / `scrollShift` / `scrollMatch` / `sharedLines`）に書き、`pipeline.json` の `result.hiddenSlides` に枚数を残す。`--scene-color 0` で色の分布の規則だけが止まる（Vision の規則は `--scene-vision` / `--scene-vision-photo`。§12.2）。ほぼ一色の画像を外す判定はどの設定でも止まらない（中身がないので、まとめではなく除外）。
...
- **呼び出しにサーバー側の打ち切りは付けない**（2026-09-18）。スリープで要求が途切れたときは、codex 自身の無応答の打ち切り（`stream_idle_timeout_ms`）と再接続に任せる。codex のこの時計は起きている時間だけを数えるので、蓋を開けて数分で自分で張り直す（10 章で、20:38 に蓋を閉じて 22:32 に開くと、codex は 22:36 に接続を張り直していた）。一方、Node のタイマーはスリープ中も進む（`process.hrtime` が起動からのスリープ込みの経過と一致する）。サーバー側で 10 分の打ち切りを足すと、期限が眠っている間に過ぎ、数分おきの短い保守用の起動（DarkWake）の途中で打ち切られる。分けた半分もそこで始まって次の短い起動で打ち切られ、やり直しがないので、長く眠ると整形がかえって失われる。一度その打ち切りを足しかけて、レビューで取りやめた（PR #22）。codex が本当に戻らない失敗が起きたら、起きている時間だけを数える安全網を考える。
- プロンプトの要点: 内容と順序を変えない、フィラーと言い直しの除去、です・ます調、専門用語はそのまま、要約や補足はしない。話題は細かく割りすぎない（10 分で 2〜4 個、90 分で 6〜15 個が目安）。
- 失敗（枠切れ、JSON 崩れ）はそのバッチだけ諦め、`notes.md` では文字起こしのまま載せる。ノート作成が全滅しても文字起こしまでは `done` にし、`pipeline.json` の `result.notesError` に理由を残す。段階が `done` のままだと利用者は `notes.md` を開くまで気づけないので（Codex の利用上限に当たったときなど）、拡張が `GET /sessions/:id/status` の `result.notes` / `result.notesError` を見て、一覧のその行に「ノートを整えられませんでした。時間をおいて「やり直す」を押してください」を出す（一部の節だけなら「ノートの一部を…」。理由はツールチップ。§11.3、2026-09-20）。失敗が混じった結果は使い回さないので、「やり直す」ではその講義のノートを全部作り直す（文字起こしはやり直さない）。ノート作成中の「中止」は失敗とは別に扱う（§11.3）: やり直しなら、やり直す前の状態に戻す。初回の処理なら、ノート作成だけを止めて文字起こしのままのノートで `done` にし、`result.notesCancelled` を付ける（拡張の注意書きは「ノート作成を中止しました（文字起こしのままです）。「やり直す」で整えられます」）。
...
サーバー未接続（トークンなし）で何も動いていないときは、サイドパネルに「このMacと接続」と説明だけを出し、Start や状態表示は出さない（2026-09-09。接続しないと文字起こしできないため。未接続では一覧も出さない）。説明には「個人の学習用です。利用するサイトの規約に従ってください。」の 1 行を含める（§3.0、2026-09-20。使い始める前に必ず通る画面のため）。

Video 行の再生位置は、検知スクリプトの報告（5 秒ごとの定期報告と、再生・停止・シークなどのイベント時。§6.6）をそのまま出すと 5 秒に 1 回しか進まないので、再生中はパネル側で報告からの経過に再生速度を掛けて足し、「経過」と同じ 0.5 秒の拍子で描き直す（2026-09-17、`videoTimeNow`）。検知スクリプトと service worker の動きは変えず、通信も保存も増やさない。停止・バッファリング・シークもイベントで報告されるので、ずれは次の報告で直る。ただし報告は service worker の直列の処理を通るので、録音中に別のセッションの送信や中止が走っている間は届くのが遅れ、止めた位置より先に進んで見えて、届いた時点で戻る（ふだんは 1 秒未満。サーバーの応答が遅いと数秒）。報告が途切れたときのために、足すのは報告から 6 秒ぶんの経過まで（定期報告の間隔 + 1 秒。足す秒数は 6 秒 × 再生速度で止まる）とし、動画の長さは超えない。同じ行の「変化 x%」は検知の判定結果なので補えず、報告のたびに変わる。行の数字は等幅にして、毎秒の書き換えで幅や折り返しが揺れないようにする。開始処理中（STARTING）は追跡する動画がまだ決まっていないので、Video 行は「動画なし（音声のみ）」ではなく「—」にする。

パネルは開いたときに、パネルのいるウィンドウの前面のタブ（＝パネルを開いたタブ）の動画を調べ、Start 前の確認として Video 行（見つからなければ NO_VIDEO の警告）に出す。パネルはページの移動をまたいで開いたままなので、移動が始まったら前のページの表示を消し、読み込みが終わったら（URL だけが変わる移動でも）調べ直す。動画プレイヤーは読み込みのあとで `<video>` を差し込むことが多いので、見つかるまで移動から 1.5・4・8・15 秒後にも調べ直す（調べるのにかかった時間は差し引く。次の移動が始まったら取りやめ、調べている間に移動したらその結果は捨てる）。調べられなかったとき（許可が切れた、内部のページ）は前の表示を消す。ほかのタブやウィンドウから戻ったときも調べ直す（2026-09-21）。この表示は覚えておき、状態や設定が変わって描き直しても消さない（2026-09-17。前の講義を文字起こししている間に次の講義のページでパネルを開くと、処理の段階が進むたびに描き直しが走り、Start 前の確認が消えていた）。Start したら捨てる。

状態別の主ボタン: IDLE = Start、CAPTURING = Stop + 「今の画面を手動で保存」、UPLOADING / PROCESSING = Server 行に段階、COMPLETED = 「出力フォルダを開く」。

セッション一覧（録音中も表示）は保存済みのセッションを新しい順に全部出す（2026-09-11。それまでは 5 件までで、3 章の録音が一覧から消えた）。講義が増えると一覧が長くなるので、**最初に見せるのは 10 件**にし、11 件目からは「すべて表示（残り N 件）」で開く（2026-09-20）。行は最初から全件を描いておき、見た目だけ畳む（`ul.collapsed li.extra` を CSS で隠す）。件数を絞って取り直すわけではないので、開閉で行を描き直さず、スクロール位置もサーバーへの問い合わせ（フォルダの有無）もそのまま。開いているときのボタンは「10 件だけ表示」。パネルを開き直すと畳んだ状態に戻る。処理中・送信待ちの行は畳まない（古い講義を「やり直す」したときに、進み具合と「中止」が見えなくならないように）。開くときだけ、出てくる行を少し上からふわっと出す（`prefers-reduced-motion` では動かさない）。10 件以下ならボタンは出さない。一覧はパネルごとスクロールする（ポップアップがあった頃は、縦が限られるので一覧の中だけを 300px でスクロールさせていた。2026-09-21 にポップアップごとやめた）。各行の 1 行目は動画ページのタイトル（古い録音で無ければ日時）、2 行目に日時・長さ・サイズ・枚数。タイトルが幅の都合で省略されているときだけ、タイトルに乗せる（かボタンに Tab で入る）とタイトル全文を出すツールチップが開き、文字は選んでコピーできる（ボタンやフォルダのパスは置かない。2026-09-11。同じ行のボタンを覆わないよう、タイトルの上（三角を含めて 6px）に出し、上に収まらないときだけ下に出す。幅は文字に合わせ、行の幅を上限にする。行の中央を指す三角付きの濃い色の吹き出しで、行のブロックと見分けられるようにする。パネルの外には出せない。タイトルから外れて 0.25 秒で消える。ツールチップは body 直下に置くので `.sessionTip[hidden]` で hidden を効かせる）。各行のボタン（2026-09-11 に「破棄」を状態ごとに分けた）: 処理済みなら「フォルダを開く」「やり直す」「非表示」「削除」、未処理（送る前・送信失敗）なら「文字起こしする」「削除」、処理中・送信待ちなら「中止」だけ。「非表示」は一覧から隠すだけで Chrome 側の録音も `~/LecScribe` のフォルダも残り、一覧の下の「非表示のセッションを表示」で戻せる（status.json の `hidden`）。「削除」はパネル内の確認ダイアログ（HTML の `<dialog>` を `showModal()` で出す。「削除する / キャンセル」。ブラウザの `confirm()` はサイドパネルで表示されずに閉じられることがあるので使わない）のあと Chrome 側の録音と `~/LecScribe` のフォルダ（notes.md があっても。`cancel { delete: true, force: true }`）を消す。「中止」も同じ形の確認（「中止する / キャンセル」）で、初回の処理なら途中のフォルダも録音も消し、やり直し中なら処理を止めるだけで前回の結果と録音は残る（`{ output: 'keep', keepRecording: true }`。やり直しでは pipeline が途中で notes.md を文字起こしそのままの版で上書きしないので、どこで中止しても前回の整えた notes.md が残る）。処理済みの行は描くときにサーバーへ `GET /sessions/:id/status` を聞き、404 なら「データなし」のタグを出して「フォルダを開く」「やり直す」を押せなくする（Finder でフォルダを消したあとの行。「削除」で片付けられる）。Start の説明文（「動画を再生した状態で Start を押してください」など状態ごとの一言）は Start ボタンの直下に置く（一覧の下では一覧が長いと見えないため）。処理中・送信待ちの行には「処理中」「送信待ち」のタグが付く。文字起こしが終わった行では、状態表示が「Done」と同じことを言うので説明文は出さない（2026-09-11）。
...
サーバー（「このMacと接続」で承認、「接続を解除」（トークンを持っているときだけ出す。確認してから解除。2026-09-18）、接続テスト）。ポートとトークンの欄は置かない（2026-09-18。ポートは §12.1d、トークンは D-10）。音声・検知・保存の項目は置かない（2026-09-09。既定のままで困らないため。config には残す）。
...
| OPFS 書込失敗 / 容量不足 | 例外 / `navigator.storage.estimate()` | 「保存できません」 | Stop してサーバーへ送る |
...
- 新しく作る出力先（`~/LecScribe`）とセッションフォルダは本人だけが読める権限（700）で作る。ホームは同じ Mac の別のアカウント（グループ `staff`）が通れる権限なので、既定の 755 だと録音やノートを読まれる。既にあるフォルダの権限は変えない（2026-09-18）。
- トークンはログに書かない。`slides.json` の画像名は拡張が付ける `slide_001.png` / `.jpg` の形だけを受け付ける（名前をパスにして ffmpeg や Vision に渡すため。2026-09-18）。
- 出力ディレクトリはユーザーのホーム配下。個人の学習用途を前提とし、録音・画像・ノートを第三者に配らない（§3.0）。
- 原案 §42 の禁止事項を維持する（§21）。
...
- MediaRecorder → Worker → OPFS。Stop でエクスポート（chrome.downloads。2026-09-24 に廃止）
...
- 初回保存、重複防止、timeline 記録、`slides.json` / `timeline.json`、エクスポート（当時。2026-09-24 に廃止）
- 完了条件: 実際の動画 1 本でスライド一式と JSON が取り出せ、`videoTime` が正しい
...
- サイト側の検知を回避するための機能、特定のサイトの規約をすり抜けるための機能（§3.0、2026-09-20）
