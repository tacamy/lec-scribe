...
- v0.8（2026-09-17）: パネルの Video 行の再生位置を、検知スクリプトの報告の間も進める（§15.1。数字を等幅にし、動画の長さで止め、開始処理中は「—」）。ポップアップの Start 前の動画の表示を、状態が変わっても消さない（§15.1）。同じ場面のまとめで、まとまりの前の方の画像とも比べ、読み取りが安定しない場面では文字を拒否の根拠にしない（§13.4b）。
