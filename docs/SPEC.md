...
- `pnpm install` は走らせない（サーバーはランタイム依存なし）。Vision の補助コマンド（§13.4b）は**サーバーが起動時に作る**（2026-09-11、#10）。`listen` のあとに待たずに走らせ、名前がソースのハッシュなので変わっていなければ `access` 1 回で済む。`install.sh` / `update.sh` では作らない（起動時のビルドと同時に走って `swiftc` が二重になる）。どの経路で更新しても起動は必ず通るので、更新の後始末はここに集める。
...
- 次に、**見た目の距離**（macOS の Vision、`VNGenerateImageFeaturePrintRequest`。2026-09-10）で比べる。画素や色の分布では分けられなかった「同じ写真の続き」「メニューを開いただけ」「少しスクロールしただけ」を、人の目に近い基準で拾う。Swift の小さな補助コマンド（`server/tools/imagefp.swift`）をサーバーの起動時に `swiftc` でビルドして `~/.lec-scribe/bin/` に置き（§12.1b。間に合わなければ最初の処理で待つ）、画像パスを渡して距離の行列を受け取る。Mac の中だけで動きトークンを使わない。`swiftc`（Xcode Command Line Tools）がなければこの判定を飛ばす。
