...
- **許可が切れたらパネルを閉じる。** 開いたタブを `chrome.storage.session` に覚えておき、そのタブの読み込みが終わったとき（`tabs.onUpdated` の `complete`）にタブの URL が読めなければ、許可が切れている（`tabs` 権限は持っていないので、URL が読めるのは `activeTab` が効いている間だけ）。何もしていないときに限って `setOptions({ tabId, enabled: false })` で閉じる。次に使うときは必ずアイコンを押すことになり、許可が新しくなる。同じサイトの中の移動では許可は残るので、閉じない。録音中・処理中・送信待ちの間は閉じない（録音は移動しても続くし、進み具合を見ているかもしれない）。
