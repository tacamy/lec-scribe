...
ポップアップからは、自分のタイミングでもサイドパネルへ切り替えられる（2026-09-21）。ポップアップは縦が限られ、「やり直す」のときなどに一覧が見づらいため。上部の「サイドパネルで開く」（アイコンのボタン。ポップアップにだけ出す）を押すと、Start のときと同じく、いま見ているタブにだけパネルを有効にして `chrome.sidePanel.open()` を呼ぶ（ユーザー操作の直後でないと開けないので `setOptions` は待たない）。パネルにフォーカスが移るので、ポップアップは閉じる。状態の正本は `chrome.storage.session` と offscreen にあるので、録音中・処理中に切り替えても何も止まらない。こうして開いたパネルからの Start は、上のとおり許可が残っている間だけ成功する（ページを移動したあとは、アイコンから開始するよう案内が出る）。上部の「設定」と「サイドパネルで開く」はアイコンだけのボタンにし、名前は `aria-label` と `title`（乗せると出る）で伝える。
...
サーバー未接続（トークンなし）で何も動いていないときは、ポップアップ / サイドパネルに「このMacと接続」と説明だけを出し、Start や状態表示は出さない（2026-09-09。接続しないと文字起こしできないため。未接続では一覧も出さない）。説明には「個人の学習用です。利用するサイトの規約に従ってください。」の 1 行を含める（§3.0、2026-09-20。使い始める前に必ず通る画面のため）。
