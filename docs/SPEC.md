...
- まず、**ほぼ一色の画像**は載せない（`reason: 'blank'`、2026-09-22）。画素の 99.5% 以上が、その画像の中央値の色から 24 以内（`uniformFraction`）。真っ黒（「はじめに」の 025。フェードの途中で撮れた）、動画の先頭の灰色 1 枚（13 章 GD の 001）、暗い画面に小さな点だけが動くコマ（7 章 GD の 060、99.7%）が該当する（060 が外れた結果、それまで 057 と「中身が同じ」でまとまっていた別の字幕の 059 が載るようになる）。白地に短い見出しだけのスライドは 28 セッションで最大 97.9%、読める文字のある暗い画面は 98.0% なので残る。比較の基準にもしない（前後の同じ場面のまとまりが一色の画像で切れないように）ので `sameSceneAs` は付かず、間の発話は今載っている画像の下に入る。
...
- 次に、**見た目の距離**（macOS の Vision、`VNGenerateImageFeaturePrintRequest`。2026-09-10）で比べる。画素や色の分布では分けられなかった「同じ写真の続き」「メニューを開いただけ」「少しスクロールしただけ」を、人の目に近い基準で拾う。Swift の小さな補助コマンド（`server/tools/imagefp.swift`）を `swiftc` でビルドして `~/.lec-scribe/bin/` に置き（常駐サーバーは起動時に作る。§12.1b。間に合わなければ最初の処理で待つ）、画像パスを渡して距離の行列を受け取る。Mac の中だけで動きトークンを使わない。`swiftc`（Xcode Command Line Tools）がなければこの判定を飛ばす。ビルドは 3 分で打ち切る（CLT が無い Mac にも `/usr/bin/swiftc` があり、「開発者ツールをインストールしますか」のダイアログを出したまま返らないことがある。打ち切らないと、あとから補助コマンドを待つ文字起こしも一緒に止まる）。**作れなかった結果は覚えない**（次に呼ばれたらやり直す。常駐サーバーは何日も動くので、起動時にたまたま駄目だっただけで、その間ずっと諦めることになる）。**覚えたパスはまだあるか確かめてから使う**（置き場を退避・削除されたら次の文字起こしで作り直す。/health の見え方と食い違わないように）。状態は `/health` の `vision` で分かり（§12.2）、拡張の設定画面の「接続テスト」と `agent.mjs install` / `status` に「見た目の判定（Vision）: あり / 準備中 / まだ作っていません / 作れませんでした（理由）」と出る（2026-09-11、#17。`install.sh` が前もってビルドしていたときはターミナルに結果が出ていたが、起動時に移して見えなくなったため）。CLT の導入を勧めるのは「作れませんでした」のときだけ。まだ作っていない・作っている途中を「なし」と一括りにすると、CLT がある Mac に導入を勧めてしまう。
  ...
  - **画面を大きくスクロールしただけの組**も外す（`reason: 'scrolled'`、2026-09-22）。上の平行移動の規則は縦 12 画素（13%）までしか探さない。Web ページをカード 1 行分（25 画素前後）以上スクロールすると探索の外になるうえ、写真を含む画面は縮小で画素がぴったり合わず、探索を縦 40 画素・320×180 まで広げても「説明できた画素」は 3〜4 割にしか届かなかった（GD I-2 の 2 章 057→058→059）。そこで画素ではなく **行ごとの平均色の並び** を見る（`rowScroll`）。
    ...
    - 平行移動と同じく基準の画像とだけ比べるので、長いページを少しずつ送り続けると、探す範囲を超えたところで次の 1 枚が残る（2 章では 056〜058 が 058 に、059 は残る）。28 セッション 1,069 枚で新しくまとまるのは 2 章の 057、5 章 GD の 028（ピクトグラムの一覧を送った）、11 章 GD の 023〜027（Illustrator のキャンバスを大きく送りながら描いた）だけ。2 章の 053→055 のように画面の半分以上が入れ替わるスクロールは重なりが 4 割しかなく、まとめない
