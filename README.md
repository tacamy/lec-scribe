...
動画を見ながらノートを取るための、汎用の Chrome 拡張です。講義や解説の動画を再生すると、話の内容と画面をノート（Markdown）にまとめます。特定のサイト向けの道具ではありません。

ページに `<video>` で埋め込まれた動画（`blob:` で再生する配信を含む）を再生すると、音声を録音し、スライドが切り替わったときだけ画面を保存します。見終わると Mac 上の WhisperKit が日本語で文字起こしし、スライドと本文を時間順に並べたノートができます。
...
- **個人の学習用です。** 自分が視聴できる動画を見ながら、自分のためのノートを作る道具です。録音・画像・ノートを人に配ったり公開したりしないでください
- **利用するサイトの規約に従ってください。** 録音や画面の保存を禁じているサイトでは使わないでください。規約と著作権を守る責任は、使う人にあります
...
2. ツールバーの LecScribe アイコンを押すと、右にサイドパネルが開きます。Mac 側の準備がまだなら、パネルに次の 1 行が出るので「ターミナル」に貼り付けて Enter を押します
...
動画ページで動画を再生してから、アイコンを押してサイドパネルを開き、Start を押します。パネルはアイコンを押したタブにだけ出ます（他のタブで状態を見たいときは、そのタブでもアイコンを押します）。別のサイトへ移動すると、何もしていなければパネルは閉じます。録音のたびにアイコンから開き直してください（録音の許可が、アイコンを押したページにだけ出るためです）。
...
[GitHub の Issues](https://github.com/tacamy/lec-scribe/issues) へどうぞ（サイドパネルの一番下にもリンクがあります）。知りたいのは、**導入のどこで詰まったか**（ターミナル・Homebrew・パスワード・待ち時間）、どのサイトの動画で使ったか、ノートの精度、ノートを整える設定までできたか、です。不具合のときは `~/Library/Logs/lec-scribe/server.log` の該当部分を添えてください。

利用状況の自動送信（テレメトリ）はありません。扱うデータと外部に出るものは [プライバシーポリシー](docs/PRIVACY.md) にまとめてあります。
