...
ツールバーのアイコンのポップアップから Start すると、サイドパネルが開いて録音中の状態を表示します。Stop すると自動でサーバーに送って文字起こしが始まります。処理中に次の動画の録音を始めてもよく、Stop 後は送信待ちに並びます。処理中のセッションを一覧の「破棄」で消すと処理も中止されます。Chrome への読み込み方と各 Phase の確認項目は [docs/CHECKS.md](docs/CHECKS.md) を参照。
